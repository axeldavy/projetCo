...
and lire_CHAINE = parse
  ...
  | caractere as c 
      ...
      ^(lire_CHAINE lexbuf)}
  | _ as c {raise(Lexing_error ("illegal character near: " ^ String.make 1 c ))}

(* Sert à lexer les caracteres pouvant etre presents dans les chaines. *)
 and char_of_character = parse
	| [ ' ' '!' '#' - '[' ']'-'_'  'a'-'~' ] as c {c}
	| "\\\\" {'\\'}
	| "\\\'" {'\''}
	| "\\\""  {'\"'}
	| "\\x" ((chiffre_hexa chiffre_hexa) as c)
