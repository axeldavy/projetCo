...
and
  ...
  |TPointer_access e -> let code = code_expr e and type1 = pointed_type (e.texp_type) in (* on suppose sortier dans A0. A completer et corriger*)
			code ++ charger_p type1 A0 A0
  ...
  |TAssignement (e1,e2) -> let code_adr = lvalue_code e1 in let type1 = e1.texp_type in
			   let code_e = code_expr e2 in (* on suppose sortier dans A0. A completer et corriger*)
			   code_adr ++ mips [Arith(Sub,SP,SP,Oimm(4))] ++ mettre_p2 4 SP A0 0 ++ code_e
			   ++ charger_p2 4 SP A1 0 ++ mettre_p type1 A1 A0 ++ mips[Arith(Add,SP,SP,Oimm(4))]
  ...
  |TCall (f,l) -> (* on suppose sortier dans A0. A completer et corriger*)
    let put_arg e =  
      (code_expr e) ++ mettre_p2 (get_size e.texp_type) SP A0 (-4) ++ (mips [Arith(Sub,SP,SP,Oimm(4))])
    in 
    let taille_resultat = 4* (size_octet e.texp_type) in
    let code1 = List.fold_left (fun acc e -> acc ++ (put_arg e)) nop l in
    (mips[Arith(Sub,SP,SP,Oimm(taille_resultat)) ]) ++ code1 ++ (mips [Jal("fun_"^f.f_name) ]++ charger_p2 (get_size f.f_type)SP A0 0 ++ mips[Arith(Add,SP,SP,Oimm(taille_resultat))])
  |TUnop (op,e) -> (begin let type1 = e.texp_type in
     match op with
    | PPleft -> let code_adr = lvalue_code e in
		code_adr ++ charger_p type1 A0 T0 ++ mips[ Arith(Add,T0,T0,Oimm(1)) ] ++ mettre_p type1 A0 T0 ++ mips[ Move(A0,T0)] 
    | PPright -> let code_adr = lvalue_code e in
		 code_adr ++ charger_p type1 A0 T0 ++ mips[ Arith(Add,T0,T0,Oimm(1)) ] ++ mettre_p type1 A0 T0 ++ mips[ Arith(Sub,A0,T0,Oimm(1))] 
    | MMleft -> let code_adr = lvalue_code e in
		code_adr ++ charger_p type1 A0 T0 ++ mips[Arith(Sub,T0,T0,Oimm(1)) ] ++ mettre_p type1 A0 T0 ++ mips[ Move(A0,T0)] 
    | MMright -> let code_adr = lvalue_code e in
		 code_adr ++ charger_p type1 A0 T0 ++ mips[Arith(Sub,T0,T0,Oimm(1)) ] ++ mettre_p type1 A0 T0 ++ mips[ Arith(Add,A0,T0,Oimm(1))] 
...
let rec code_instr = function
  ...
  | TIf(e,i1,None) -> let code_e = code_expr e in
		      ...
		      let code_i1 = code_instr i1.tinstr in
  ...
  | TIf(e,i1,Some i2) -> let code_e = code_expr e in
			 ...
			 let code_i1 = code_instr i1.tinstr and code_i2 = code_instr i2.tinstr in
