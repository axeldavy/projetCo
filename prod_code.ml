...
let rec get_size = function
	...
	| TStruct s -> if (s.s_size <> 0) then s.s_size  
		...
		if (aligne var.lv_type)
			then
				( var.lv_pos <- acc +((4 - (acc mod 4)) mod 4) ; (*un peu moche, renvoie la position alignée juste au dessus de acc.
	...
	| TUnion u -> if (u.u_size <> 0) then u.u_size  
		else (u.u_size <- List.fold_left (fun acc var -> var.lv_pos <- 0 ; max acc (get_size var.lv_type)) 0 u.u_content ; u.u_size) ;
...
let set_loc_instr pos = function 
	| TEmpty | TExpr _ | TReturn _ -> pos ;
	| TIf(_,i1,i2_opt) -> begin let pos1 = set_loc_instr pos i1 in 
	...
	| TBloc b -> 
		let aux acc decvar =  
			...
			in
			let size = get_siz var.lv_type in 
			let new_pos = assert (size_up_to_date var.lv_type) ;
				pos - size - ((4 - (size mod 4)) mod 4) 
			in 
			var.lv_pos <- new_pos ;
			new_pos 
		in 
...
let set_loc p = 
	let aux = function 
		...
		| TDf df -> let posf = set_loc_inst (-4) df.bloc in df.tfun.f_lvar_size <- 4 + (abs posf)
			(*on suppose que tous les arguments de la fonction sont alignés sur la pile !!! (plus simple)*)
				let res_pos = List.fold_left (fun pos_libre var -> var.lv_pos <- pos_libre ; 
					assert (size_up_to_date var) ;
					let size = get_size var.lv_type in 
					pos_libre + size + ((4 - (size mod 4)) mod 4 )) 4 df.tfun.f_arg
...
let code_decl = function 
	| TDt _ -> () (*pour une déclaration de type, il n'y a aucun code à produire*)
	| TDvar _ -> () (*les déclarations de variables vont dans le champ data*)
...
(*manque certainement un bout du genre jump main et gestion des arguments du programme*)
	{text = mips (List.map code_decl p.tdecl) ; data = data_decl p.tdecl}